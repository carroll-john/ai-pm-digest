...
- **`headline`** — punchy. Include emoji + author/medium when relevant: `🎙 Lenny's Podcast: …`, `🎥 Claire Vo: …`, `🐦 Shreyas Doshi: …`, `🤖 Anthropic ships …`.
...
**Top-level fields:**
- **`subject`** — `🤖 AI × PM Daily — Sat 9 May` (use today's date)
- **`date_label`** — `Sat 9 May`
- **`greeting`** — `Good morning John,`
- **`intro`** — one short line, e.g. `Here's what's moving in AI for Product Managers today.`
- **`reflection`** — one sharp question or uncomfortable observation tying the day together. Not a summary.
- **`sign_off`** — `Stay curious,\nYour AI Digest`

**"Try it" examples:**
- ✅ "Paste your current sprint backlog into Claude and ask: 'What's the highest-risk item and why?'"
- ❌ "Open Claude, paste your backlog, then ask it to identify the highest-risk item — this exercise will help you apply the framework above to your own work."
